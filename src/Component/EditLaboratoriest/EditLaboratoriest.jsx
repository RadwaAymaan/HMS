...
import styles from './EditLaboratoriest.module.css'
export default function EditLaboratoriest() {
  ...
  let{LaboratoriestDetails,editLaboratoriest} = useContext(LaboratoriestContext)

  async function updateLaboratoriest(id,values){
    setLoading(true)  
    let {data} = await editLaboratoriest(id,values);
    console.log(data);
    if (data.message === 'success')
        {
          ...
          navigate(`/Admin/LaboratoriestDetails/${parmas.id}`)
        }
   }

   async function callLaboratoriestDetails(id){
    let{data} = await LaboratoriestDetails(id)
    setuser(data.Laboratoriest.userId)
    ...
    useEffect(()=>{
      callLaboratoriestDetails(parmas.id)
    },[])
  ...
  enableReinitialize:true,
    onSubmit:(values)=>{updateLaboratoriest(parmas.id,values)}
    })
  ...
  <div className=' bg-opacity-75 bg-light p-3  m-5 '>
    <h3 className='fw-bold text-center text-primary text-opacity-75 p-2'>Update Laboratoriest</h3>
    ...
    <form onSubmit={formik.handleSubmit} >
      ...
      <div className="w-50 m-auto p-1 ">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Edit Laboratoriest</button>}
       </div>
