...
import styles from './RadiologistDetails.module.css'
import { useNavigate, useParams } from 'react-router-dom';
import { RadiologistContext } from '../../Context/RadiologistContext';
export default function RadiologistDetails() {

  let {RadiologistDetails}=useContext(RadiologistContext)
  ...
  async function viewDetails(id){
    ...
    setDetails(data.radiologist)
    setuserData(data.radiologist.userId)
  ...
  <div className="h-75 bg-opacity-75 bg-light mt-5 m-auto p-1">
      ...
      <div className="w-50 p-2 m-auto  text-center mt-5">
         ...
         <button onClick={()=>{
          navigate(`/Admin/EditRadiologist/${parmas.id}`)
         }}   className='btn  btn-outline-primary fw-bold text-black'>Edit Radiologist</button>
          </div>  
