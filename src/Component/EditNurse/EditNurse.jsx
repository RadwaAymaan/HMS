...
import styles from './EditNurse.module.css'
export default function EditNurse() {
  let navigate =useNavigate()
  let parmas = useParams()
  let{nurseDetails,editNurse} = useContext(NurseContext)
  ...
  async function updateNurse(id,values){
    setLoading(true)  
    let {data} = await editNurse(id,values)
    console.log(data);
    if (data.message === 'success')
        {
          ...
          navigate(`/Admin/NurseDetails/${parmas.id}`)
        }
     
   }
   async function callNurseDetails(id){
    let{data} = await nurseDetails(id)
    setNurse(data.Nurse)
    setuser(data.Nurse.userId)
    setSalary(data.Nurse.Salary)
    setday(data.Nurse.Times.Days)
    settime(data.Nurse.Times.Time)
    console.log(data.Nurse.Times.Days);
   
   }
    useEffect(()=>{
     callNurseDetails(parmas.id)
    },[])
    ...
    validationSchema,
    onSubmit:(values)=>{updateNurse(parmas.id,values)}
    }) 
  ...
  return<>
    ...
    <form onSubmit={formik.handleSubmit} >
      ...
      <div className="w-50 m-auto p-1 ">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Edit Nurse</button>}
       </div>
