...
import styles from './AddLaboratoriest.module.css'
...
export default function AddLaboratoriest() {
  ...
  let{handleAddLaboratoriest} = useContext(LaboratoriestContext)

  async function addLaboratoriest(values){
    setLoading(true)  
    let {data} = await handleAddLaboratoriest(values);
    console.log(data);
    if (data.message === 'success')
        {
          ...
          navigate('/Admin/AllLaboratoriests')
        }
   }
   ...
   nameLaboratory:Yup.string().required('nameLaboratory is required'),
  ...
  <div className=' bg-opacity-75 bg-light p-3  m-5 '>
    <h3 className='fw-bold text-center text-primary text-opacity-75 p-2'>Add Laboratory</h3>
    ...
    <form onSubmit={formik.handleSubmit} >
     ...
     <select onBlur={formik.handleBlur} className="form-select mb-2 " name="Gender" id='Gender'  value={formik.values.Gender} onChange={formik.handleChange}>
      ...
      <div className="w-50 p-2">
      <label className='fw-bold' htmlFor="nameLaboratory">name Laboratory</label>
      <input onBlur={formik.handleBlur} className='form-control mb-2 ' type="text" name='nameLaboratory' id='nameLaboratory' value={formik.values.nameLaboratory} onChange={formik.handleChange}/>
      {formik.errors.nameLaboratory && formik.touched.nameLaboratory ?<div className='alert alert-danger'>{formik.errors.nameLaboratory}</div>:null}
      </div>
      ...
      <div className="w-50 m-auto  p-1 ">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Add Laboratory</button>}
       </div>
