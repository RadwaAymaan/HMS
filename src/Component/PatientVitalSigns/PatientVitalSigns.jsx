...
import styles from './PatientVitalSigns.module.css'
export default function PatientVitalSigns() {
  let{patientVitalSigns} = useContext(DoctorContext)
  ...
  let{data} = await patientVitalSigns(id)
  ...
  <div className="h-75 bg-opacity-75 bg-light mt-5 m-auto p-1 rounded">
    ...
    <div className='col-md-6 d-flex '>
          ...
          <span className='ms-2'>{time.slice(0,1)}<span>{(parseInt((Number(time.slice(0,2))) + Number(3)))}</span>{time.slice(2,5)}</span>
           </h2>
          </div> 
          </div>
