...
import styles from './PatientXrayReport.module.css'
import { useContext } from 'react'
import { PatientContext } from '../../Context/PatientContext'
export default function PatientXrayReport() {
  let {patientXray} = useContext(PatientContext)
  const [report, setreport] = useState([])

  async function Report() {
    let {data}=await patientXray();
    console.log(data);
  ...
  <div className=' m-auto mt-5 '>
   ...
   {report.map((rep,index)=>(
    ...
    <tr  key={index} >
     ...
     <td  key={index+4} className='fs-5'>{rep.createdBy.name}</td>
