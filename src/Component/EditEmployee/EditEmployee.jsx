...
export default function EditEmployee() {
  ...
  let{employeeDetails,editEmployee} = useContext(EmployeeContext)

  async function updateEmployee(id,values){
    setLoading(true)  
    let {data} = await editEmployee(id,values);
    console.log(data);
    if (data.message === 'success')
        {
          ...
          navigate(`/Admin/EmployeeDetails/${parmas.id}`)
        }
   }

   async function callEmployeeDetails(id){
    ...
    enableReinitialize:true,
      onSubmit:(values)=>{updateEmployee(parmas.id,values)}
      })
  ...
  return<>
   <div className=' bg-light p-3 bg-opacity-75 m-5'>
    <h3 className='fw-bold text-center text-primary text-opacity-75 p-2'>Update Employee</h3>
    ...
    <form onSubmit={formik.handleSubmit} >
      ...
      <div className="mx-5 p-1">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Edit Employee</button>}
       </div>
