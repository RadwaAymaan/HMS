...
export default function AddDoctor() {
  ...
  let{handleAddDoctor} = useContext(DoctorContext)
 
  async function addDoctor(values){
   setLoading(true)  
   let {data} = await handleAddDoctor(values);
   console.log(data);
  ...
  <div className=' bg-light p-3 bg-opacity-75 m-5'>
    <h3 className='fw-bold text-center text-primary text-opacity-75 p-2'>Add Doctor</h3>
    ...
    <form onSubmit={formik.handleSubmit} >
     ...
     <select onBlur={formik.handleBlur} className="form-select mb-2 " name="Gender" id='Gender'  value={formik.values.Gender} onChange={formik.handleChange}>
      ...
      <div className="mx-5 p-1">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Add Doctor</button>}
       </div>
