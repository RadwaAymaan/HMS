...
import styles from './AddRadiologist.module.css'
export default function AddRadiologist() {
  ...
  let{handleAddRadiologist} = useContext(RadiologistContext)

  async function addRadiologist(values){
    setLoading(true)  
    let {data} = await handleAddRadiologist(values);
    console.log(data);
    if (data.message === 'success')
        {
          ...
          navigate('/Admin/AllRadiologists')
        }
   }
   ...
   let validationSchema=Yup.object({
    ...
    nameX_RayCenter:Yup.string().required('nameX_RayCenter is required'),
  ...
  <div className=' bg-opacity-75 bg-light p-3  m-5 '>
    <h3 className='fw-bold text-center text-primary text-opacity-75 p-2'>Add Radiologist</h3>
    ...
    <form onSubmit={formik.handleSubmit} >
     ...
     <select onBlur={formik.handleBlur} className="form-select mb-2 " name="Gender" id='Gender'  value={formik.values.Gender} onChange={formik.handleChange}>
      ...
      <div className="w-50 p-2">
      <label className='fw-bold' htmlFor="nameX_RayCenter">nameX RayCenter</label>
      <input onBlur={formik.handleBlur} className='form-control mb-2 ' type="text" name='nameX_RayCenter' id='nameX_RayCenter' value={formik.values.nameX_RayCenter} onChange={formik.handleChange}/>
      {formik.errors.nameX_RayCenter && formik.touched.nameX_RayCenter ?<div className='alert alert-danger'>{formik.errors.nameX_RayCenter}</div>:null}
      </div>
      ...
      <div className="w-50 m-auto  p-1 ">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Add Radiologist</button>}
       </div>
