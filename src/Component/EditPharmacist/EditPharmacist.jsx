...
import styles from './EditPharmacist.module.css'
...
export default function EditPharmacist() {
  ...
  let{editPharmacist,PharmacistDetails} = useContext(PharmacistContext)
  ...
  async function updatePharmacist(id,values){
    setLoading(true)  
    let {data} = await editPharmacist(id,values)
    ...
    if (data.message === 'success')
        {
          ...
          navigate(`/Admin/PharmacistDetails/${parmas.id}`)
        }
     
   }
   async function callPharmacistDetails(id){
    let{data} = await PharmacistDetails(id)
    setDoctor(data.pharmacist)
    setuser(data.pharmacist.userId)
    setSalary(data.pharmacist.Salary)
    setday(data.pharmacist.Times.Days)
    settime(data.pharmacist.Times.Time)
   }
    useEffect(()=>{
      callPharmacistDetails(parmas.id)
    },[])
    ...
    onSubmit:(values)=>{updatePharmacist(parmas.id,values)}
    })
    const addTime = () => {
      formik.setValues({
        ...formik.values,
  ...
  <div className=' bg-opacity-75 bg-light p-3  m-5 '>
    <h3 className='fw-bold text-center text-primary text-opacity-75 p-2'>Edit Pharmacy</h3>
    ...
    <form onSubmit={formik.handleSubmit} >
      ...
      <div className="w-50 m-auto  p-1 ">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Edit Pharmacy</button>}
       </div>
