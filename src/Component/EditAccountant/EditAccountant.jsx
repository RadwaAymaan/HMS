...
import styles from './EditAccountant.module.css'
export default function EditAccountant() {
  ...
  let{AccountantDetails,editAccountant} = useContext(AccountantContext)

  async function updateAccountant(id,values){
    setLoading(true)  
    let {data} = await editAccountant(id,values);
    console.log(data);
    if (data.message === 'success')
        {
          ...
          navigate(`/Admin/AccountantDetails/${parmas.id}`)
        }
   }

   async function callAccountantDetails(id){
    let{data} = await AccountantDetails(id)
    setuser(data.Accountant.userId)
    ...
    useEffect(()=>{
      callAccountantDetails(parmas.id)
    },[])
    ...
    enableReinitialize:true,
      onSubmit:(values)=>{updateAccountant(parmas.id,values)}
      })
 ...
 <div className=' bg-opacity-75 bg-light p-3  m-5 '>
    <h3 className='fw-bold text-center text-primary text-opacity-75 p-2'>Update Accountant</h3>
    ...
    <form onSubmit={formik.handleSubmit} >
      ...
      <div className="w-50 m-auto p-1 ">
       ...
       <button disabled={!(formik.isValid && formik.dirty)} type='submit' className='btn  btn-outline-primary fw-bold text-black'>Edit Accountant</button>}
       </div>
